...
def save_to_gcs(tool_context: ToolContext, image_bytes, filename: str, counter: str):
    """Save image to GCS and return status."""
    try:
        ...
        return {
            "status": "success",
            "gcs_uri": gcs_uri,
            "message": f"Image uploaded to {gcs_uri}"
        }

    except Exception as e_gcs:
        error_msg = f"Image generated but failed to upload to GCS: {e_gcs}"
        print(f"✗ GCS Upload Error: {error_msg}")
        return {
            "status": "error",
            "message": error_msg,
        }