...
# Global variable to store the driver instance (lazy initialization)
_driver = None


def get_chrome_driver():
    """Create and return a Chrome WebDriver instance with unique user data directory.
    
    This function implements lazy initialization - the driver is only created when
    it's actually needed, not when the module is imported.
    
    Returns:
        selenium.webdriver.Chrome: Configured Chrome WebDriver instance, or None if disabled.
    """
    global _driver
    ...
    # Return existing driver if already created
    ...
    temp_dir = tempfile.mkdtemp(prefix=f"chrome_user_data_{unique_id}_")
    ...
    try:
        _driver = selenium.webdriver.Chrome(options=options)
        ...
        try:
            options = Options()
            options.add_argument("--headless")  # Run in headless mode as fallback
            options.add_argument("--no-sandbox")
            options.add_argument("--disable-dev-shm-usage")
            options.add_argument("--disable-gpu")
            options.add_argument(f"--user-agent={random.choice(user_agents)}")
            _driver = selenium.webdriver.Chrome(options=options)
            print("✅ Chrome WebDriver initialized successfully (headless fallback)")
            return _driver
...
def go_to_url(url: str) -> str:
    ...
    Note:
        This function uses the Chrome driver instance to navigate to the URL.
